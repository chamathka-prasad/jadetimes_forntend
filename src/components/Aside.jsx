...
import { navPaths, mobileNavPaths } from "../routes/Paths";
...
const Aside = ({ handleMenuClose, isMenu }) => {
  ...
  const [isKey, handleKey] = useKey(null);
    ...
    <aside className={`bg-transparent duration-300 fixed top-0 bottom-0 left-0 right-0 z-50 ${isMenu ? "visible" : "invisible"}`} onClick={handleToggleMenuItem}>
      <nav className={`bg-neutral-900 text-white max-w-80 h-full relative duration-300 ${isMenu ? "translate-x-0" : "-translate-x-full"}`} onClick={stopHandleToggleMenu}>
        <div className="flex flex-col p-4 overflow-auto h-full lg:py-4 lg:px-9" ref={scrollTopRef}>
          {screen !== "large" && <Login className="text-sm my-[0.6rem] self-end" onClick={handleToggleMenuItem} />}
          ...
          <ul className="lg:mt-0">
            ...
            {navPaths.map((path) => (
              <li className={`lg:text-sm lg:border-b lg:border-b-neutral-800 ${path.submenus ? "flex flex-row flex-wrap" : ""}`} key={path.key}>
                ...
                {path.submenus && (
                  <button onClick={() => handleKey(path.key)} aria-label="sub menu" aria-expanded={isKey === path.key}>
                    {isKey === path.key ? <BsChevronUp /> : <BsChevronDown />}
                  </button>
                )}
                {path.submenus && (
                  <ul className={`w-full text-[#8B8B8B] duration-300 ${isKey === path.key ? "max-h-screen" : "max-h-0 overflow-y-hidden invisible"}`}>
          ...
          {screen !== "large" && (
            <>
              <ul className="border-t border-[#8b8b8b] mt-3 pt-5">
              ...
              <div className="mt-2">&copy; 2024 Jadetimes Media LLC. All&nbsp;Rights Reserved</div>
            </>
          )}
        </div>
