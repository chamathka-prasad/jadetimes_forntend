        ...
        {articles.slice(currentIndex * pageItems - pageItems, pageItems * currentIndex).map((article, index) => (
          <article className="flex flex-row gap-4 border border-neutral-300 p-4 items-center lg:items-start lg:grid lg:grid-cols-4" key={index}>
            ...
            <div className="col-span-3">
              ...
              <div>
                <Link className="hidden lg:block lg:w-fit lg:text-xs">{article.authorName}</Link>
                <div className="hidden lg:block whitespace-nowrap lg:text-xs"><span>{article.posted}</span><span className="w-[2px] h-[2px] bg-neutral-900 inline-block mx-2 align-middle"></span><span>{article.readTime}</span></div>
              </div>
            </div>
          </article>
        ))}
      ...
      {lastIndex !== 1 && (
        <div className="flex flex-row gap-8 items-center justify-center">
          <button
            ...
            className={`hidden lg:block ${currentIndex <= 3 || lastIndex === 5 ? "text-neutral-500" : ""}`}
          >
          ...
          <button
            ...
            className={`lg:hidden ${currentIndex === 1 ? "text-neutral-500" : ""}`}
          >
            <BsChevronDoubleLeft size={20} />
          </button>
          ...
          <div className="w-20 text-center text-lg lg:hidden">
          ...
          <button
            ...
            className={`lg:hidden ${currentIndex === lastIndex ? "text-neutral-500" : ""}`}
          >
          ...
          <button
            ...
            className={`hidden lg:block ${currentIndex >= lastIndex - 2 || lastIndex === 5 ? "text-neutral-500" : ""}`}
          >
            <BsChevronDoubleRight size={20} />
          </button>
        </div>
