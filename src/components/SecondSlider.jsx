...
const SecondSlider = () => {
  ...
  return (
    <div className="col-[span_14_/_span_14]">
      {error ? (
        <div className="aspect-video bg-red-50 border border-red-200 text-[#FF332E] flex flex-col gap-4 items-center justify-center">
        ...
        {error ? (
          <div className="grid grid-cols-4 gap-5 mt-5 w-full">
            {Array.from({ length: 4 }, (_, index) => index).map((_, index) => (
              <div key={index} className="aspect-video bg-red-50 border border-red-200 text-sm text-[#FF332E] flex flex-col items-center justify-center">{error.message}</div>
            ))}
          </div>
        ) : (
          <>
      ...
      <div className="relative flex mt-6">
        {currentIndex !== 0 && (
          <button className={`flex flex-row flex-nowrap items-center gap-3 text-sm`} onClick={handlePreviousSlide} disabled={error}>
            <BsArrowLeft size={24} />
            Prev
          </button>
        )}
        {currentIndex !== youTubeVideos.length - 1 && (
          <button className={`flex flex-row flex-nowrap items-center gap-3 text-sm ml-auto`} onClick={handleNextSlide} disabled={error}>
            Next
