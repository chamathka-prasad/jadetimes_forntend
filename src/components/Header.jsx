  ...
  return (
    <header className="bg-neutral-900 text-white sticky top-0 z-40">
      <div className="px-4 py-[1.45rem] flex items-center relative max-w-[1407px] mx-auto lg:py-[1.032rem]">
        ...
        <button className="block order-1 mr-auto lg:mr-0" onClick={onToggleMenu} aria-label="open the menu">
        ...
        <Link to="/log-in" className="hidden lg:block lg:order-4 lg:mr-8 lg:text-right lg:text-[0.813rem] lg:ml-auto">
          Log In
        </Link>
      </div>
      <nav className="hidden bg-neutral-50 text-[#111111] lg:block">
        <ul className="flex flex-row flex-wrap gap-7 p-3 px-4 justify-center text-[0.813rem] font-semibold">
          {navPaths.map((path) => (
            <li key={path.key}>
              <NavLink className="lg:duration-300 lg:hover:text-[#FF322F]" to={path.to}>
                {path.name}
              </NavLink>
            </li>
          ))}
